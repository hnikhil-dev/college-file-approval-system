...
export default function PresidentDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch files pending president's approval
      const pendingFiles = await getFilesByRole('president', currentUser.id);
  ...
  const handleApprovalSubmit = async (remark: string) => {
    ...
    try {
      if (approvalAction === 'approve') {
        await approveByPresident(selectedFile.id, user.id, remark);
        toast.success('File approved successfully!');
      } else {
        await rejectByPresident(selectedFile.id, user.id, remark);
    ...
    <>
      ...
      <main className="container-main">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-1">Final approval authority for all files</p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-8">
          <div className="card">
            ...
            <p className="text-3xl font-bold text-blue-600">{files.filter((f) => f.current_stage === 'president' && f.status === 'pending').length}</p>
          </div>
          <div className="card">
            <p className="text-sm text-gray-600 mb-1">Approved</p>
            <p className="text-3xl font-bold text-green-600">{files.filter((f) => f.current_stage === 'completed' && f.status === 'approved').length}</p>
          </div>
          <div className="card">
          ...
          {files.length === 0 ? (
            ...
            files.map((file) => (
              <FileCard
                ...
                onApprove={file.current_stage === 'president' ? () => handleApprove(file.id) : undefined}
                onReject={file.current_stage === 'president' ? () => handleReject(file.id) : undefined}
                showActions={file.current_stage === 'president'}
              />
            ))
          )}
        </div>
